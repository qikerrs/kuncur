...
import { DEFAULT_FFMPEG_TIMEOUT, DEFAULT_ORIGIN, MAX_STDERR_SIZE, MEDIA_HKDF_KEY_MAPPING, MEDIA_PATH_MAP, NEWSLETTER_MEDIA_PATH_MAP } from '../Defaults/index.js';
...
const createFfmpegWrapper = () =>
    (content = {}) =>
        ffmpegQueue.add(async () => {
            if (!Array.isArray(content.args)) {
                throw new Boom('Missing args in ffmpeg content');
            }
            if (content.args.some(arg => String(arg).includes('pipe'))) {
                throw new Boom('Invalid pipe argument for streaming process');
            }
            const ext = (content.extension || 'jpg').replace(/[^a-z0-9]/gi, '');
            const outputFilename = normalize(
                join(getTmpFilesDirectory(), `${generateMessageIDV2()}.${ext}`)
            );
            let ff,
                timeout,
                stderrChunks = [],
            ...
            try {
                ...
                }, content.timeoutMs || DEFAULT_FFMPEG_TIMEOUT);
                ...
                const [code] = await once(ff, 'close');
                if (code !== 0)
                    throw new Boom(`Ffmpeg failed (code ${code}):\n${Buffer.concat(stderrChunks).toString('utf8', 0, MAX_STDERR_SIZE)}`);
                return {
                    outputFilename,
                    toBuffer: async () => {
                        try {
                            return await fs.readFile(outputFilename);
                        } finally {
                ...
                await fs.unlink(outputFilename).catch(() => { });
                throw err instanceof Boom ?
                    err :
                    new Boom('Ffmpeg processing failed', { data: err });
            } finally {
...
export const getImageProcessingLibrary = async () => {
    ...
    const libraries = {};
    if (sharp) {
        libraries.sharp = sharp;
    }
    if (jimp) {
        libraries.jimp = jimp;
    }
    isHasFfmpeg ??= await checkFfmpeg();
    if (isHasFfmpeg) {
        libraries.ffmpeg = createFfmpegWrapper();
    }
    if (sharp || jimp || isHasFfmpeg) {
...
export const extractVideoThumb = async (path, time, size) => {
    ...
    if (!lib.ffmpeg)
        throw new Boom('FFmpeg not available');
...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    else if ('ffmpeg' in lib && typeof lib.ffmpeg === 'function') {
        // Lia@Changes 31-01-26 --- Use ffmpeg via spawn as a fallback when sharp and jimp are not available
        if (Buffer.isBuffer(bufferOrFilePath)) {
            await bufferToFile(bufferOrFilePath);
        }
        const job = await lib.ffmpeg({
            args: [
                '-i', bufferOrFilePath,
...
export const bufferToFile = async (buffer, extension = 'jpg') => {
    if (!Buffer.isBuffer(buffer)) {
        throw new Boom('Invalid media input');
    }
    const ext = extension.replace(/[^a-z0-9]/gi, '');
    const outputFilename = join(getTmpFilesDirectory(), `${generateMessageIDV2()}.${ext}`);
...
export async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        try {
            thumbnail = await extractVideoThumb(file, '00:00:00', { width: 32, height: 32 });
        }
        catch (err) {
